...
function _buildHTML(ctx) {
  ...
  <div class="studio-bottom">
    ...
    <div class="tl-wrap" id="timeline">
      ${SECTIONS.map((s,i) => `
        <div class="tl-seg ${s === 'main' ? 'main' : ''} ${s === 'main' ? 'active' : ''}"
             data-section="${s}">${s.toUpperCase()}</div>
      `).join('')}
    </div>
...
function _wireTopButtons(container) {
  ...
  container.querySelector('#btn-regen-all')?.addEventListener('click', () => {
    _onRegenLayer('all');
  });
...
async function _onRegenLayer(layer) {
  const bp = getBlueprint();
  if (!bp) return;
  const randomness = (parseInt(document.querySelector('#randomness-slider')?.value || 30)) / 100;
  const newBp = getFallbackBlueprint(_ctx.artist.id, _ctx.album, _ctx.mood);

  // Blend with randomness factor
  const target = layer === 'all' ? newBp : (layer === 'drums' ? newBp : newBp);
  await regenLayer(layer === 'all' ? 'drums' : layer, target);
  if (layer === 'all') {
...
function _startVisualizer(canvas) {
  ...
  const accent = getComputedStyle(document.documentElement).getPropertyValue('--accent').trim() || '#cc0000';
...
function _showLoadModal(container) {
  ...
  modal.style.cssText = `
    ...
    justify-content:center; gap:12px; z-index:7000;
  `;

...
export async function initEngine(blueprint) {
  ...
  const snareMerge = new Tone.Merge();
  snareNoise.connect(snareMerge.left);
  snareTone.connect(snareMerge.right);
  snareMerge.connect(_channels.snare);
...
function _disposeAll() {
  ...
  _sequences = {}; _instruments = {}; _channels = {}; _effects = {};
  _analyser = null; _initialized = false;
}
